...
def init_video(fullscreen):
    pygame.font.init()
    # respect env if set; else try common backends
    order = [os.environ["SDL_VIDEODRIVER"]] if "SDL_VIDEODRIVER" in os.environ else []
    order += ["kmsdrm","fbcon","x11","directfb","dummy"]
    tried=[]
    for drv in order:
        tried.append(drv)
        ...
        try:
            ...
            return screen, drv, False
        except pygame.error:
            pygame.display.quit()
            if drv=="dummy":
                # headless surface so you can run over SSH
                return pygame.Surface((800,480)), drv, True
    raise SystemExit("No SDL video driver available. Tried: "+",".join(tried))
